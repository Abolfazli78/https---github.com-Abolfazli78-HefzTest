...
export function GalleryBase({ images }: GalleryBaseProps) {
  ...
  return (
    <section
      className="w-full overflow-x-hidden pb-24 sm:pb-0 min-h-[70vh] md:min-h-[80vh] lg:min-h-[92vh] lg:grid lg:grid-cols-[35%_65%]"
      dir="rtl"
    >
      <div className="order-2 lg:order-1 h-full flex flex-col px-4 sm:px-6 md:px-8 py-6 sm:py-8 md:py-10">
        <div>
          ...
          <div>
            ...
            {active?.description ? (
              <p className="mt-3 sm:mt-4 text-sm sm:text-lg leading-7 sm:leading-8 text-slate-700 dark:text-slate-300">
                {active.description}
              </p>
            ) : null}
            {active?.notes?.length ? (
      ...
      <div className="order-1 lg:order-2 w-full bg-white flex items-center justify-center p-0 md:p-2">
        <button
          type="button"
      ...
      <div className="sm:hidden fixed inset-x-0 bottom-0 z-50 border-t bg-white/95 dark:bg-slate-950/95 backdrop-blur px-4 py-3 shadow-lg">
        <div className="flex items-center justify-between gap-3">
          <Button type="button" variant="outline" className="flex-1" onClick={goPrev} disabled={disabled}>
            <ChevronRight className="h-4 w-4" />
            قبلی
          </Button>
          <Button type="button" className="flex-1" onClick={goNext} disabled={disabled}>
            بعدی
            <ChevronLeft className="h-4 w-4" />
          </Button>
        </div>
