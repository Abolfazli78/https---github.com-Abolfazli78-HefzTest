    ...
    <div className="min-h-screen bg-white dark:bg-slate-950 overflow-hidden">
      ...
      <section className="relative pt-20 pb-32 md:pt-32 md:pb-48">
        ...
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-7xl h-full -z-10">
          ...
          <div className="absolute inset-0 opacity-20 blur-3xl -z-10" style={{ background: 'radial-gradient(60% 60% at 70% 20%, rgba(79,70,229,0.25), transparent 60%), radial-gradient(50% 50% at 20% 80%, rgba(16,185,129,0.18), transparent 60%)' }} />
        </div>
            ...
            <div className="grid md:grid-cols-2 gap-10 items-center">
              <div className="text-center md:text-right">
                ...
                <h1 className="text-5xl md:text-7xl font-extrabold tracking-tighter mb-6 bg-gradient-to-r from-indigo-700 via-indigo-500 to-purple-500 dark:from-indigo-300 dark:via-indigo-200 dark:to-white bg-clip-text text-transparent leading-[1.1]">
                  آینده‌ی سنجش و ارزیابی در دستان شما
                </h1>
                    ...
                    >
                      ...
                      {!isLoggedIn && (
                        <motion.span
                          ...
                          style={{ background: "linear-gradient(115deg, transparent 0%, rgba(255,255,255,0.25) 50%, transparent 100%)" }}
                        />
                      )}
                    </Button>
                  </Link>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-6 gap-6 items-center opacity-80 grayscale">
            {['المهدی','الکوثر','نور','المبین','حکمت','طه'].map((name, i) => (
              <div key={i} className="h-12 rounded-xl border border-slate-200 dark:border-slate-800 flex items-center justify-center bg-slate-50/60 dark:bg-white/5">
      ...
      <section className="relative py-20">
        <div className="pointer-events-none absolute -z-10 inset-0">
          ...
          <div className="absolute inset-0 opacity-20 blur-3xl -z-10" style={{ background: 'radial-gradient(60% 60% at 80% 20%, rgba(79,70,229,0.25), transparent 60%), radial-gradient(50% 50% at 20% 80%, rgba(45,212,191,0.18), transparent 60%)' }} />
        </div>
        <div className="container mx-auto px-4">
...
function IconFeature({ icon, title, desc }: { icon: React.ReactNode; title: string; desc: string }) {
  return (
    <div className="p-6 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white/70 dark:bg-white/5 backdrop-blur shadow-sm hover:shadow-lg transition-all">
      <div className="mb-4 inline-flex items-center justify-center h-11 w-11 rounded-xl bg-gradient-to-br from-indigo-50 to-blue-50 ring-1 ring-inset ring-indigo-200/60">
        {icon}
      </div>
...
function DashboardPreview() {
  return (
    ...
    >
      <div
        className="absolute inset-0 rounded-[24px] border border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-white/10 shadow-2xl backdrop-blur-xl"
        ...
        <div className="h-full w-full p-6 grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <ul className="space-y-2 text-sm">
              {["مهدی رضایی", "فاطمه امینی", "زهرا احمدی", "حسین نظری"].map((n, i) => (
                <li key={i} className="flex items-center justify-between">
                  <span className="flex items-center gap-2"><span className="h-6 w-6 rounded-full bg-indigo-500/20" />{n}</span>
          ...
          <div className="md:col-span-3 rounded-xl border border-slate-200 dark:border-slate-800 p-4 bg-white/90 dark:bg-white/5">
            <div className="flex items-center justify-between">
              ...
              <Button size="sm" className="rounded-full">مشاهده</Button>
            </div>
          </div>
        </div>
      </div>
...
function FeatureCard({ icon, title, description }: { icon: React.ReactNode; title: string; description: string }) {
  return (
    <motion.div
      whileHover={{ y: -5 }}
      className="p-8 rounded-3xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-xl transition-all"
    >
      <div className="mb-6 p-3 bg-slate-50 dark:bg-slate-900 w-fit rounded-2xl">
        {icon}
      </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 mb-4">
        <div className="h-10 w-10 rounded-xl bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300 flex items-center justify-center">
          {icon}
        </div>
      ...
      <ul className="space-y-2 text-slate-600 dark:text-slate-400 text-sm">
        {points.map((p, i) => (
          <li key={i} className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-emerald-500" />{p}</li>
        ))}
      </ul>
    </div>
  );
