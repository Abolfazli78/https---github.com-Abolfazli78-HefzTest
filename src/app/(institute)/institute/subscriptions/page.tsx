...
const featureLabelMap: Record<string, string> = {
  ...
  teacher_management: "مدیریت معلمان",
  advanced_analytics: "تحلیل پیشرفته",
  white_label: "برند سفید",
...
export default function InstituteSubscriptionsPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const [plansRes, subscriptionRes] = await Promise.all([
        fetch("/api/subscriptions/plans?targetRole=INSTITUTE"),
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      <div className="mb-8 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black mb-2">اشتراک‌های پنل موسسه</h1>
          <p className="text-muted-foreground">پلن مناسب موسسه را انتخاب کنید و ارتقاء دهید</p>
        </div>
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <Sparkles className="h-4 w-4" />
          اشتراک‌ها فقط برای نقش موسسه نمایش داده می‌شوند
        </div>
      </div>
              ...
              <CardContent className="space-y-6">
                <div>
                  ...
                  <div className="grid gap-2 text-sm text-muted-foreground">
                    ...
                    {plan.maxTeachersAllowed !== undefined && (
                      <div className="flex items-center justify-between">
                        <span>معلمان</span>
                        <span className="font-bold text-foreground">{isUnlimited(plan.maxTeachersAllowed) ? "نامحدود" : plan.maxTeachersAllowed}</span>
                      </div>
                    )}
